...
for (string arm : arms)
{
	NewPad("$\xi$ (FN mean)", "mean of $(\xi_{\rm F} - \xi_{\rm N})$");
	...
	for (int dsi : datasets.keys)
	{
		...
		RootObject obj = RootGetObject(f, alignment+"/"+cut_option+"/p_xi_" + arm + "_FNdiff_vs_xi_" + arm + "_FNmean");

		string label = replace(datasets[dsi], "_", "\_");

		draw(obj, "eb", StdPen(dsi), label);
	}

	limits((0.05, -0.005), (0.15, 0.005), Crop);
}
